...
Uses scikit-learn (+ XGBoost if available) for automatic training and evaluation.
Fully skinned with premium dark glassmorphism SaaS tokens.
"""
...
def run_automl(df: pd.DataFrame, target_col: str, feature_cols: Optional[list] = None) -> dict:
    ...
    try:
        if hasattr(best_model, "feature_importances_"):
            ...
            fi = dict(sorted(zip(feature_cols, imps), key=lambda x: x[1], reverse=True)[:AUTOML_MAX_FEATURES_SHOWN])
...
def render_automl_page() -> None:
    ...
    with st.expander("⚙️ AutoML Pipeline Hyper-Configuration", expanded=True):
        ...
        target_col = c1.selectbox(
            "Select target output variable", all_cols,
            index=all_cols.index(suggested) if suggested in all_cols else len(all_cols) - 1,
        )
        feature_cols = c2.multiselect(
            "Select training feature vectors", [c for c in all_cols if c != target_col],
...
def _render_automl_results(results: dict) -> None:
    ...
    # Dynamic KPI test split metrics rendering
        ...
        if st.button("✨ Extract Comprehensive Insights For This Model Run", key="automl_ai", use_container_width=True):
            with st.spinner("Gemini is interpreting validation telemetry profiles..."):
                interp = generate_automl_reasoning(
                    results["task"], results["target"],
                    results["cv_scores"], results["feature_importance"],
                )
            st.markdown(
